...
for products in products_all:
    ...
    for reagents in reagents_all:
        ...
        f = open("../xml/miss_react.xml", "a")
        # print content to xml
        f.write('<!-- question: 0  -->\n')
        f.write('<question type=\"category\">\n')
        f.write('<category>\n')
        f.write('<text>$course$/Default for Chemistry/Reactant/' + str(q_type).title() + '</text>')
        f.write('</category>\n')
        f.write('</question>\n\n')
        ...
        f.write('<name>\n')
        f.write('<text>')
        f.write('Question Name: Reactant ' + str(q_type).title())
        f.write('</text>\n')
        f.write('</name>\n')
